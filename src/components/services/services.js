document.write(
  '    <div class="container">\
<h2>Наши Услуги</h2>\
<p class="services_text">\
  Мы предоставляем полный комплекс услуг по изготовлению пресс-форм и\
  изделий из пластмасс на собственном парке ТПА\
</p>\
<ul class="services-list">\
  <li class="services-item">\
    <a href="">\
      <div class="services-item_description">\
        <span class="services-item_description__text">Ремонт и модификация пресс-форм</span>\
        <svg\
        class="services-item_description__svg"\
          width="18"\
          height="31"\
          viewBox="0 0 23 38"\
          fill="white"\
          xmlns="http://www.w3.org/2000/svg"\
        >\
          <path\
            d="M22.1973 17.3048L5.49193 0.599707C5.10555 0.213026 4.58977 0 4.03981 0C3.48985 0 2.97407 0.213026 2.5877 0.599707L1.35746 1.82964C0.556935 2.63108 0.556935 3.93365 1.35746 4.73387L15.3854 18.7618L1.34189 32.8053C0.955518 33.192 0.742188 33.7075 0.742188 34.2571C0.742188 34.8074 0.955518 35.3229 1.34189 35.7099L2.57213 36.9395C2.95881 37.3262 3.47429 37.5392 4.02425 37.5392C4.57421 37.5392 5.08999 37.3262 5.47636 36.9395L22.1973 20.2191C22.5846 19.8312 22.7974 19.3133 22.7961 18.7627C22.7974 18.21 22.5846 17.6924 22.1973 17.3048Z"\
                     />\
        </svg>\
      </div>\
    </a>\
  </li>\
  <li class="services-item">\
    <a href="">\
      <div class="services-item_description">\
        <span class="services-item_description__text">Ремонт и модификация пресс-форм</span>\
        <svg\
          width="18"\
          height="31"\
          viewBox="0 0 23 38"\
          fill="#FFFFFF"\
          xmlns="http://www.w3.org/2000/svg"\
        >\
          <path\
            d="M22.1973 17.3048L5.49193 0.599707C5.10555 0.213026 4.58977 0 4.03981 0C3.48985 0 2.97407 0.213026 2.5877 0.599707L1.35746 1.82964C0.556935 2.63108 0.556935 3.93365 1.35746 4.73387L15.3854 18.7618L1.34189 32.8053C0.955518 33.192 0.742188 33.7075 0.742188 34.2571C0.742188 34.8074 0.955518 35.3229 1.34189 35.7099L2.57213 36.9395C2.95881 37.3262 3.47429 37.5392 4.02425 37.5392C4.57421 37.5392 5.08999 37.3262 5.47636 36.9395L22.1973 20.2191C22.5846 19.8312 22.7974 19.3133 22.7961 18.7627C22.7974 18.21 22.5846 17.6924 22.1973 17.3048Z"\
                   />\
        </svg>\
      </div>\
    </a>\
  </li>\
  <li class="services-item">\
    <a href="">\
      <div class="services-item_description">\
        <span class="services-item_description__text">Ремонт и модификация пресс-форм</span>\
        <svg\
          width="18"\
          height="31"\
          viewBox="0 0 23 38"\
          fill="#FFFFFF"\
          xmlns="http://www.w3.org/2000/svg"\
        >\
          <path\
            d="M22.1973 17.3048L5.49193 0.599707C5.10555 0.213026 4.58977 0 4.03981 0C3.48985 0 2.97407 0.213026 2.5877 0.599707L1.35746 1.82964C0.556935 2.63108 0.556935 3.93365 1.35746 4.73387L15.3854 18.7618L1.34189 32.8053C0.955518 33.192 0.742188 33.7075 0.742188 34.2571C0.742188 34.8074 0.955518 35.3229 1.34189 35.7099L2.57213 36.9395C2.95881 37.3262 3.47429 37.5392 4.02425 37.5392C4.57421 37.5392 5.08999 37.3262 5.47636 36.9395L22.1973 20.2191C22.5846 19.8312 22.7974 19.3133 22.7961 18.7627C22.7974 18.21 22.5846 17.6924 22.1973 17.3048Z"\
                />\
        </svg>\
      </div>\
    </a>\
  </li>\
  </li>\
   <li class="services-item">\
    <a href="">\
      <div class="services-item_description">\
        <span class="services-item_description__text">Ремонт и модификация пресс-форм</span>\
        <svg\
          width="18"\
          height="31"\
          viewBox="0 0 23 38"\
          fill="#FFFFFF"\
          xmlns="http://www.w3.org/2000/svg"\
        >\
          <path\
            d="M22.1973 17.3048L5.49193 0.599707C5.10555 0.213026 4.58977 0 4.03981 0C3.48985 0 2.97407 0.213026 2.5877 0.599707L1.35746 1.82964C0.556935 2.63108 0.556935 3.93365 1.35746 4.73387L15.3854 18.7618L1.34189 32.8053C0.955518 33.192 0.742188 33.7075 0.742188 34.2571C0.742188 34.8074 0.955518 35.3229 1.34189 35.7099L2.57213 36.9395C2.95881 37.3262 3.47429 37.5392 4.02425 37.5392C4.57421 37.5392 5.08999 37.3262 5.47636 36.9395L22.1973 20.2191C22.5846 19.8312 22.7974 19.3133 22.7961 18.7627C22.7974 18.21 22.5846 17.6924 22.1973 17.3048Z"\
                     />\
        </svg>\
      </div>\
    </a>\
  </li>\
  </li>\
</ul>\
</div>\
'
);
